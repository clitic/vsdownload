...
def capture(url: str = typer.Argument(..., help="website url to target"),
            ...
            driver: str = typer.Option(None, help=f"path of chrome driver for selenium"),
            scan_ext: str = typer.Option("ts", help="scan network logs until --scan-ext extension is found in any one of url")):
...
def save(input: str = typer.Argument(..., help="url | .m3u8 | log.json"),
        output: str = typer.Option("merged.ts", "--output", "-o", help="path for output for downloaded video stream file"),
        threads: int = typer.Option(1, "--threads", "-t", help="download multiple ts files in parallel threads"),
        blob: str = typer.Option(None, "--blob", "-b", help="endpoint base url for all segments"),
        user_agent: str = typer.Option(None, help="by default make a request to url with default user agent header"),
        ...
        timeout: int = typer.Option(5, help="waiting time for post tasks to perform after downloading stream (in seconds)"),
